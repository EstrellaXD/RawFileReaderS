...
/// </typeparam>
public class ConcurrentItemCache<T> where T : class, IDeepCloneable<T>
{
	/// <summary>
	...
	private readonly ConcurrentQueue<QueueItem> _itemCache;
	...
	/// </param>
	public ConcurrentItemCache(int capacity, ItemReader reader)
	{
		_cacheLimit = capacity;
		_reader = reader;
		_itemCache = new ConcurrentQueue<QueueItem>();
	}
	...
	public T GetItem(int index)
	{
		...
		if (val != null)
		{
			...
			{
				_itemCache.TryDequeue(out var _);
			}
	...
	{
		QueueItem result;
		while (_itemCache.TryDequeue(out result))
		{
		}
	}
}
