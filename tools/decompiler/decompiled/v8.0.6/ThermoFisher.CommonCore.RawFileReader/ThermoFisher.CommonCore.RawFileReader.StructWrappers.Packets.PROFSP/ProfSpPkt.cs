...
/// </summary>
internal class ProfSpPkt : SimplePacketBase, IMsPacket, IPacket, IRawObjectBase
{
	...
	/// The viewer.
	/// </param>
	/// <param name="dataOffset">offset of scan in memmory reader</param>
	...
	public ProfSpPkt(IMemoryReader viewer, long dataOffset, IMsScanIndexAccess scanIndex, int fileRevision, bool isSingleScan = false)
	{
	...
	private List<SegmentData> ExpandProfileBlob()
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			for (int j = 1; j < totalPackets; j++)
			{
				if ((array[j] & 0x80000000u) != 0)
				{
					list2.Add(new DataPeak(num2, array[j] & 0xFFFFFFF));
					num3++;
				}
				num2 += massTick;
