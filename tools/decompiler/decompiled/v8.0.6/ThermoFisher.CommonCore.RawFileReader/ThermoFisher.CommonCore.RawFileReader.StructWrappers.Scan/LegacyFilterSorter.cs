...
/// </summary>
internal static class LegacyFilterSorter
{
	/// <summary>
	...
	{
		QsortSlice(values, 0, values.Count - 1);
	}
	...
	public static void QsortSlice(IList<FilterScanEvent> values, int firstIndex, int lastIndex)
	{
		...
		while (true)
		{
			...
			else
			{
				...
				if (values[num2].CompareTo(values[num5]) > 0)
				{
					Swap(num2, num5, values);
				}
				if (values[num2].CompareTo(values[num3]) > 0)
				{
					Swap(num2, num3, values);
				}
				if (values[num5].CompareTo(values[num3]) > 0)
				{
				...
				while (true)
				{
					if (num5 > num6)
					{
						...
						while (num6 < num5 && values[num6].CompareTo(values[num5]) <= 0);
					}
					if (num5 <= num6)
					{
						do
						...
						while (num6 <= num3 && values[num6].CompareTo(values[num5]) <= 0);
					}
					do
					{
						num7--;
					}
					while (num7 > num5 && values[num7].CompareTo(values[num5]) > 0);
					if (num7 < num6)
					{
						break;
					}
				...
				if (num5 >= num7)
				{
					...
					while (num7 > num2 && values[num7].CompareTo(values[num5]) == 0);
				}
	...
	private static void ShortSort(int low, int high, IList<FilterScanEvent> values)
	{
		while (high > low)
		{
			...
			{
				if (values[i].CompareTo(values[num]) > 0)
				{
					num = i;
				}
			}
